...
func getTypeMap(schema client.Schema) (map[string]fieldMetadata, metadata) {
	...
	for name, field := range schema.ResourceFields {
		...
		} else if strings.EqualFold(field.Type, "hostname") || strings.EqualFold(field.Type, "dnsLabel") || strings.EqualFold(field.Type, "dnsLabelRestricted") || strings.EqualFold(field.Type, "password") {
			fieldMetadataMap[fieldName] = fieldMetadata{"String", field.Required}
...
func generateType(packageName string, schema client.Schema) error {
	return generateTemplate(schema, path.Join(SOURCE_OUTPUT_DIR, "type", packageName, ToFirstUpper(schema.Id)+".java"), packageName, "type.template")
}
